...
`;
  ...
  const ps1Content = `$host.ui.RawUI.WindowTitle = 'Hours Worked Tracker Server'
Set-Location -Path "$PSScriptRoot\\server"

Write-Host ""
Write-Host "========================================"
Write-Host "  Hours Worked Tracker Server v${version}"
Write-Host "========================================"
Write-Host ""
...
`;
  ...
  const installerName = `${brandId}_HoursWorkedTracker_Server_Setup_${version}`;
  const nsisScript = `
; NSIS Installer Script for Hours Worked Tracker Server
; Generated by build-server-installer.js

!include "MUI2.nsh"

; General
Name "Hours Worked Tracker Server ${version}"
OutFile "${OUTPUT_DIR}\\${installerName}.exe"
; Install to C:\HoursWorkedTracker by default (not Program Files) to avoid permission issues with SQLite database
InstallDir "C:\\HoursWorkedTracker"
InstallDirRegKey HKCU "Software\\HoursWorkedTracker" "InstallDir"
...
Section "Install"
  ...
  WriteRegStr HKCU "Software\\HoursWorkedTracker" "InstallDir" "$INSTDIR"

  ; Create uninstaller
  WriteUninstaller "$INSTDIR\\Uninstall.exe"

  ; Create Start Menu shortcuts
  CreateDirectory "$SMPROGRAMS\\Hours Worked Tracker"
  CreateShortcut "$SMPROGRAMS\\Hours Worked Tracker\\Start Server.lnk" "$INSTDIR\\Start Server.bat" "" "$INSTDIR\\node\\node.exe"
  CreateShortcut "$SMPROGRAMS\\Hours Worked Tracker\\Uninstall.lnk" "$INSTDIR\\Uninstall.exe"

  ; Create Desktop shortcut
  CreateShortcut "$DESKTOP\\Hours Worked Tracker.lnk" "$INSTDIR\\Start Server.bat" "" "$INSTDIR\\node\\node.exe"
...
Section "Uninstall"
  ...
  ; Remove shortcuts
  Delete "$SMPROGRAMS\\Hours Worked Tracker\\Start Server.lnk"
  Delete "$SMPROGRAMS\\Hours Worked Tracker\\Uninstall.lnk"
  RMDir "$SMPROGRAMS\\Hours Worked Tracker"
  Delete "$DESKTOP\\Hours Worked Tracker.lnk"

  ; Remove registry keys
  DeleteRegKey HKCU "Software\\HoursWorkedTracker"
